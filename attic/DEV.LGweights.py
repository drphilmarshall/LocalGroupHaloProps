...
import LGconstraints,TimingArgument,NFW
...
def LGweights(argv):
  ...
  USAGE
    local_group_weights.py [flags] [options] pairs.dat

  FLAGS
          -h               Print this message [0]
          -v               Be verbose
          -t               Only do triplets calculations (fast!)
  ...
  EXAMPLES
     ...
     LGweights.py -c constraints.txt  *halos*.txt
  ...
  try:
      opts, args = getopt.getopt(argv[1:], "hvt", ["help","verbose","triplets-only"])
  except getopt.GetoptError, err:
      # print help information and exit:
      print str(err) # will print something like "option -a not recognized"
      print LGweights.__doc__
      sys.exit(2)
  ...
  # quantities.
     ...
     for input in inputs:
       ...
       consuelo_id = numpy.zeros(len(t),dtype='|S2')
       consuelo_id.fill(run)
       t.add_column('run',consuelo_id)
     ...
     # Now have all tables read in and combined into 3, one for each object.
     ...
     # Shift all halos to galactocentric coordinates, subtracting off M31
     ...
     distances_and_velocities(MW,M33) 
     ...
     # Write out pairs and triplets for future use.
     # Weights are added at this point:  
  ...
  if do_pairs:
     ...
     if fail:
       print "Warning: pairs FITS tables have inconsistent lengths:"
  ...
  # Add column with MW-M31 timing argument mass, to MW tables: 

  if do_pairs: 
    timing_argument(MWp) 
  timing_argument(MWt) 

  # Add column with M33-M31 timing argument mass, to M33 tables: 

  timing_argument(M33t) 

  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  
  # Start out with prior weight (all ones except a few zeros):
  if do_pairs: wp0 = MWp.w * M31p.w
  ...
  if do_pairs:   
     ...
     # Compute MW-M31 distance constraint, and save the posterior:
     wp1 = wp0*obs.Nd_likelihood(MWp,['D'])
     filename = 'LG_pairs_MW_D.cpt'
     write_cpt_file(pindex,MWp,M31p,M31p,wp1,np,filename,vb)

     # Compute MW-M31 radial velocity constraint, and save the posterior:
     wp2 = wp0*obs.Nd_likelihood(MWp,['vr'])
     filename = 'LG_pairs_MW_vr.cpt'
     write_cpt_file(pindex,MWp,M31p,M31p,wp2,np,filename,vb)

     # Compute MW-M31 tangential velocity constraint, and save the posterior:
     wp3 = wp0*obs.Nd_likelihood(MWp,['vt'])
     filename = 'LG_pairs_MW_vt.cpt'
     write_cpt_file(pindex,MWp,M31p,M31p,wp3,np,filename,vb)

     # Combine all MW-M31 kinematic constraints, and save the posterior:
     wp4 = wp0*obs.Nd_likelihood(MWp,['D','vr','vt'])
     filename = 'LG_pairs_MW_D+vr+vt.cpt'
     write_cpt_file(pindex,MWp,M31p,M31p,wp4,np,filename,vb)
...
#   # Compute MW-M31 distance constraint, and save the posterior:
#   wt1 = wt0*obs.Nd_likelihood([MWt],['D'])
#   print "wt1 = ",wt1,wt1.shape
#   filename = 'LG_triplets_MW_D.cpt'
#   write_cpt_file(tindex,MWt,M31t,M33t,wt1,nt,filename,vb)
# 
#   # Compute MW-M31 radial velocity constraint, and save the posterior:
#   wt2 = wt0*obs.Nd_likelihood([MWt],['vr'])
#   filename = 'LG_triplets_MW_vr.cpt'
#   write_cpt_file(tindex,MWt,M31t,M33t,wt2,nt,filename,vb)
# 
#   # Compute MW-M31 tangential velocity constraint, and save the posterior:
#   wt3 = wt0*obs.Nd_likelihood([MWt],['vt'])
#   filename = 'LG_triplets_MW_vt.cpt'
#   write_cpt_file(tindex,MWt,M31t,M33t,wt3,nt,filename,vb)

  # Combine all MW-M31 kinematic constraints, and save the posterior:
  wt4 = wt0*obs.Nd_likelihood([MWt],['D','vr','vt'])
  filename = 'LG_triplets_MW_D+vr+vt.cpt'
...
def distances_and_velocities(MW,M33):
  
  D = numpy.sqrt(MW.x*MW.x + MW.y*MW.y + MW.z*MW.z)
  ii = numpy.where(D <= 0.0)
  if len(ii[0]) > 0:
    print "Error: M31-MW distance is zero or negative:"
    for i in ii[0]:
      print MW[i]
#       D[i] = 0.75
    sys.exit()
  ...
  ii = numpy.where(D <= 0.0)
  if len(ii[0]) > 0:
    print "Error: M31-M33 distance is zero or negative:"
    for i in ii[0]:
      print M33[i]
#       D[i] = 0.75
    sys.exit()
  ...
  # get vt:
  v = numpy.sqrt(MW.vx*MW.vx + MW.vy*MW.vy + MW.vz*MW.vz)
  vr = (MW.vx*MW.x + MW.vy*MW.y + MW.vz*MW.z)/MW.D
  costheta = vr/v
...
def timing_argument(neighbor):
      
  TimingArgument.vb = False
  M,a,chi,e = TimingArgument.mass(neighbor.D,neighbor.vr,approach='radial',t0scatter=True)
  
  neighbor.add_column('M_TA',M)
...
# Now parse target filename and select parameters to plot:  
  ...
  if stem == 'pairs':
    ...
    hline1 = '# importance, ' + logM_MW_label + logM_M31_label \
                     ...
                     + logM_LG_pair_label \
                     + logM_TA_label + logA200_label + vt_M31_label
    hline2 = '# 0,1,        ' + logM_MW_range + logM_M31_range \
                     + D_M31_range + vr_M31_range + logMratio_range \
                     + logM_LG_pair_range \
                     + logM_TA_range + logA200_range + vt_M31_range
    ...
    outbundle[:,11] = numpy.log10(M31.M_TA[index])
    outbundle[:,12] = numpy.log10(M31.M_TA[index]/(MW.M200[index]+M31.M200[index]))
    outbundle[:,13] = M31.vt[index]
