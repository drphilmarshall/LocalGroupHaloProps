...
hlist_path = base_path+str(box_num)+'/'+str(box_num)+'bgc2.npy'
if vmax:
    vmax_cut=vmax
else:
    vmax_cut=None
...
host_flag = np.where(halos['pid'] == -1)[0]
...
larger_host, larger_host_sub, smaller_host, smaller_host_sub = iso_criteria.criteria.isolated(pairs, halos, host_flag, iso_dist, M33_dist, box_size, vmax_cut, periodic=True)

dat_M31_larger = iso_criteria.criteria.get_data(halos, host_flag, smaller_host, larger_host, larger_host_sub, smaller_host_sub)
dat_MW_larger = iso_criteria.criteria.get_data(halos, host_flag, larger_host, smaller_host, smaller_host_sub, larger_host_sub)
quad_dat_M31_larger = iso_criteria.criteria.get_quad_data(dat_M31_larger)
quad_dat_MW_larger = iso_criteria.criteria.get_quad_data(dat_MW_larger)
...
#    np.save(save_path+'dat_M33_larger_vres80.npy', dat_M33_larger)
    np.save(save_path+'quad_dat_M31_larger_bgc2.npy', quad_dat_M31_larger)
    np.save(save_path+'quad_dat_MW_larger_bgc2.npy', quad_dat_MW_larger)
